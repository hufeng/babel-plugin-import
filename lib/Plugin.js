  ...
  _createClass(Plugin, [{
    ...
    value: function importMethod(methodName, file, opts) {
      if (!this.selectedMethods[methodName]) {
        ...
        var meta = this.resources[methodName];
        if (!meta) {
          throw new Error('Could not find ' + methodName + ' meta data');
        }
        var modulePath = meta.path;
        var isDefault = meta['default'];
        ...
        this.selectedMethods[methodName] = isDefault ? file.addImport(path, 'default') : file.addImport(path, methodName);
