...
var Plugin = function () {
  function Plugin(libraryName, libraryDirectory, moduleResolver, types) {
    ...
    this.libraryDirectory = libraryDirectory == void 0 ? 'lib' : libraryDirectory;
  ...
  _createClass(Plugin, [{
    ...
    value: function importMethod(methodName, file, opts) {
      if (!this.selectedMethods[methodName]) {
        ...
        //获取当前模块名的Resolver信息
        var resolver = this.moduleResolver[methodName];
        if (!resolver) {
          throw new Error('Could not find ' + methodName + ' resolver data in .babelrc');
        }

        //当前模块映射的路径
        var modulePath = resolver.path;
        if (!modulePath) {
          throw new Error('Could not find ' + methodName + ' resolver path field in .babelrc');
        }

        //default|*|export
        var moduleType = resolver.type;
        if (!moduleType) {
          throw new Error('please specify ' + methodName + '\'s resolver type.');
        }

        var transformedMethodName = modulePath;
    ...
    key: 'NewExpression',
    value: function NewExpression(path, _ref11) {
